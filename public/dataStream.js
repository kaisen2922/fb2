...
(function() {
  ...
  window.initializeRpaStream = async function(callback, csvUrl = '/rpa_database_2026.csv') {
    ...
    try {
      ...
      console.log("🚀 [Pipeline Engine] Starting lower-frequency background execution firehose...");

      // Telemetry firehose tick rate reduced to preserve runtime frame rate
      setInterval(() => {
        if (memoryPool.length === 0) return;

        // Keep the stream gentle enough to preserve UI responsiveness during the dashboard run
        const batchSize = 1;
        const incomingBatch = [];
